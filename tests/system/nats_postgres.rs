...
use support::{
    EffectHandler, Fixture, OrderCreated, Receipt, Running, effect_count, receipt,
    run_with_cleanup, settings, wait_for_receipt,
};
...
async fn handler_effect_commits_with_the_receipt_before_acknowledgement() {
    ...
    run_with_cleanup(ack_wait, move |fixture| async move {
        let Fixture {
            pool,
            broker,
            pull_consumer,
