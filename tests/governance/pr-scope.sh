...
has_pull_request="$(jq --exit-status --raw-output 'if has("pull_request") then "true" else "false" end' <<<'{"number":35,"state":"open"}')"
[[ "${has_pull_request}" == false ]] || fail "JSON issue adapter did not produce raw false"
issue_is_open_not_pull_request 35 35 open "${has_pull_request}" || fail "JSON issue adapter rejected valid issue"
has_pull_request="$(jq --exit-status --raw-output 'if has("pull_request") then "true" else "false" end' <<<'{"number":35,"state":"open","pull_request":{}}')"
[[ "${has_pull_request}" == true ]] || fail "JSON issue adapter did not produce raw true"
issue_is_open_not_pull_request 35 35 open "${has_pull_request}" && fail "JSON pull request adapter accepted PR"
