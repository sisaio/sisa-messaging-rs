...
meets G3 is the one explicit exception to the final gate: the owning writer fixes it, no reviewer is
spawned, the primary verifies the fix hunks, and the handoff and PR record the reviewed HEAD, the
final HEAD, and the changed paths. Approval then covers the reviewed HEAD plus that primary-verified
delta. Other medium/low final findings are deferred to a linked follow-up issue. Database/migration
fixes keep section 6 focused re-review (P2).
...
config may qualify and proceed directly to fresh final review (G3); after a final review, G1's
exception applies instead.
...
uninspected scope plus open risk. Consume summarized broad-check evidence; never rerun tests,
Clippy, `cargo deny`, or a full diff already evidenced.
...
HEAD change invalidates approval except the recorded G1 small-fix delta; focused review never
replaces final review. The gate requires the recorded range to remain current, no blocker/high,
every accepted medium fixed or dispositioned, required checks and CI evidenced, docs and behavior
aligned, and no unresolved schema or public compatibility decision.
