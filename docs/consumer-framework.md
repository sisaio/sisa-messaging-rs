...
- `nak_delay` is the broker redelivery delay for retryable/in-progress work. It must be non-zero
  in broker mode until the zero-delay retry policy (#11) exists, and is unused in pending-recovery
  mode.
- `drain_timeout` bounds the whole graceful drain after receiving stops.
- `mode` selects broker settlement (the default) or pending recovery (section 5). It is always an
  explicit application choice and is never inferred from a source descriptor.
- A `heartbeat_interval` setting, non-zero and below half a source-reported ack wait, is deferred
  to #11 together with heartbeat acknowledgement.
...
For long individual-delivery handlers, the planned `heartbeat_interval` (#11) must be configured
below half the broker's acknowledgement wait. The source reports its descriptor during `run`
opening, so requirements are validated after I/O but before receiving. Inbox `max_attempts` must
not exceed a finite reported `max_deliver`; otherwise the broker can stop delivery before the
inbox records its dead transition.

## 5. Individual-delivery state machine

The application selects one of two explicit settlement modes through `ConsumerSettings::mode`;
the runtime never infers a mode from a source descriptor. Both modes share one workflow and one
centralized decision table, and both keep the same guarantees: delivery is at least once, the
inbox deduplicates redelivery, and a delivery is never acknowledged before its transaction
commits.
...
7. A malformed individual delivery with no trustworthy message identity cannot safely create an
   inbox row; it is terminated only in broker mode, which requires terminal discard. A malformed
...
capability is tracked by #80. RabbitMQ does not use this mode; it uses the zero-delay retry mode
planned in #11.
...
| Resolution | Broker | Pending recovery |
| --- | --- | --- |
| Committed success, completed duplicate, or completed elsewhere | ack | ack |
| In progress, or retryable failure rolled back and recorded | delayed nak | leave pending |
| Transient begin, claim, completion, or commit failure (commit ambiguity) | delayed nak | leave pending |
...
on the source, while every permit is taken. Heartbeat acknowledgement is deferred to #11.
