...
- Require independent `reviewer` review after every material Rust, SQL, migration, test,
  manifest/dependency, CI/release, normative-document, `.codex/config.toml`, or agent-config
  change. A fresh read-only final reviewer checks the committed `merge-base...HEAD` diff and
  records current base/head SHAs, path count, checks, and range secret scan. A changed base or head
  invalidates approval, and focused review cannot replace final review.
